...
parser.add_argument('--dataroot', default='data/face/train/', help='path to trn dataset')
parser.add_argument('--valDataroot', default='data/face/test/', help='path to val dataset')
...
parser.add_argument('--evalIter', type=int, default=300, help='interval for evauating(generating) images from valDataroot')
...
val_target_cpu, val_input_cpu = val_target_cpu.to(device), val_input_cpu.to(device)
...
# NOTE training loop
ganIterations = 0
k = 0.0 # control how much emphasis is put on L(G(z_D)) during gradient descent.
...
for epoch in range(opt.niter):
  ...
  for i, data in enumerate(dataloader, 0):
    ...
    balance = (opt.gamma * errD_real - errD_fake).item()
    k = min(max(k + opt.lambda_k * balance, 0), 1)
    measure = errD_real.item() + np.abs(balance)
    ...
    if ganIterations % opt.display == 0:
      print('[%d/%d][%d/%d] Ld: %f Lg: %f Limg: %f, M_global: %f(%f), K: %f, balance.: %f lr: %f'
          % (epoch, opt.niter, i, len(dataloader),
             errD.item(), errG.item(), L_img.item(),
             measure, M_global.avg, k, balance,
             optimizerG.param_groups[0]['lr']))
      sys.stdout.flush()
      trainLogger.write('%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n' % \
                        (i, errD.item(), errG.item(), L_img.item(), measure, M_global.avg, k, balance))
      trainLogger.flush()
